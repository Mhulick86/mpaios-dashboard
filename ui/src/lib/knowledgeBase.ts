...
export function buildKnowledgeContext(
  ...
  if (entries.length === 0) {
    return "\n\n## Knowledge Base (Agent 18 — Institutional Memory)\n*No entries yet — this is a new knowledge base.*\n\nIMPORTANT: You MUST output [LEARNING:category:confidence] markers at the end of your response to populate the knowledge base. Every substantive conversation should produce at least one learning. This is how the system gets smarter over time.\n";
  }
...
export function parseLearnings(content: string): {
  ...
  // Flexible: handles newlines, extra whitespace, backtick wrapping, code blocks, and optional tags section
  // Strip backticks and code fences before matching
  const cleaned = content.replace(/```[\s\S]*?```/g, (block) => {
    // Preserve learning markers inside code blocks by unwrapping them
    if (block.includes("[LEARNING:") && block.includes("[/LEARNING]")) {
      return block.replace(/^```\w*\n?/, "").replace(/\n?```$/, "");
    }
    return block;
  }).replace(/`(\[LEARNING:[\s\S]*?\[\/LEARNING\])`/g, "$1");

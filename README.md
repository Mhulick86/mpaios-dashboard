...
- Ad, local-SEO, CRM and messaging platforms (Google Ads, Google Business Profile, LinkedIn, TikTok, Pinterest, HubSpot, Slack, Semrush) connect from the Platforms grid on `/integrations`; tokens are exchanged and stored server-side, encrypted with `INTEGRATIONS_ENCRYPTION_KEY`. Developer-console setup, redirect URIs, scopes and env vars per platform: [docs/integrations.md](docs/integrations.md).
